...
const ProductList = () => {
  ...
  const cartItems = useSelector(state=>state.cart.cartItems); //get card items globaly

  const products = [
    { id: 1, name: 'Product A', price: 60, quantity: 0 },
    { id: 2, name: 'Product B', price: 75, quantity: 0 },
    ...
    <div className="product-list">
      ...
      {products.map(product => (
        <li key={product.id} className='product-list-item'>
          ...
          <button
              className={`add-to-cart-btn ${cartItems.some(item => item.id === product.id) ? 'disable' : ''}`}
